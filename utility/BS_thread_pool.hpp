...
class [[nodiscard]] multi_future {
 public:
  ...
  [[nodiscard]] std::vector<T> get() {
    ...
    for (size_t i = 0; i < f.size(); ++i) results[i] = f[i].get();
    return results;
  }
   ...
   */
  void wait() const {
    for (size_t i = 0; i < f.size(); ++i) f[i].wait();
  }
...
class [[nodiscard]] thread_pool {
 public:
   ...
   * submitting each block separately to the queue.
   *
   ...
   * index_after_last; ++i)". Note that if first_index == index_after_last, no
  ...
  [[nodiscard]] multi_future<R> parallelize_loop(const T1& first_index,
    ...
    for (size_t i = 0; i < num_blocks; ++i) {
      ...
      const T end =
          (i == num_blocks - 1)
              ? index_after_last_T
              : (static_cast<T>((i + 1) * block_size) + first_index_T);
